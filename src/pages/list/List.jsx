...
const List = () => {
  ...
  useEffect(() => {
    if (user) {
      fetch(`https://craft-place-server-2.vercel.app/allitems/${user.email}`)
        .then((res) => res.json())
  ...
  const handleDelete = (userItem) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`https://craft-place-server-2.vercel.app/item/${userItem._id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
            ...
            if (data.deletedCount > 0) {
              fetch(
                `https://craft-place-server-2.vercel.app/allitems/${user.email}`
              )
                .then((res) => res.json())
                .then((data) => setItems(data));
              Swal.fire({
